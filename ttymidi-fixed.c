/*
    ...
    *********************
    Additional Info
    *********************
    The original ttymidi application did not support sysex messages. This
   version does. Based it on the work of johnty/ttymidi-icubex.c (see
   https://gist.github.com/johnty/de8b3d3041c7ee43accd)

    The other change from original ttymidi code is that the MIDI por type being
   created: the bit SND_SEQ_PORT_TYPE_MIDI_GENERIC was added so that the virtual
   port shows up when searching for ports in ofxMidi
...
int open_seq(snd_seq_t **seq) {
  int port_out_id,
      port_in_id; // actually port_in_id is not needed nor used anywhere
  ...
  if ((port_out_id = snd_seq_create_simple_port(
           *seq, "MIDI In", SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ,
           SND_SEQ_PORT_TYPE_MIDI_GENERIC | SND_SEQ_PORT_TYPE_APPLICATION)) <
      0) {
    fprintf(stderr, "Error creating sequencer port.\n");
  }

  if ((port_in_id = snd_seq_create_simple_port(
           *seq, "MIDI Out",
...
void write_midi_action_to_serial_port(snd_seq_t *seq_handle) {
  ...
  unsigned char sysex_data[256];
  int sysex_len = 0;

  do {
    snd_seq_event_input(seq_handle, &ev);
    sysex_len = 0; // reset each iteration so stale SysEx isn't re-sent
    ...
    case SND_SEQ_EVENT_KEYPRESS:
      bytes[0] = 0xA0 + ev->data.control.channel;
      bytes[1] = ev->data.note.note;
      bytes[2] = ev->data.note.velocity;
      if (!arguments.silent && arguments.verbose)
    ...
    case SND_SEQ_EVENT_PGMCHANGE:
      ...
      bytes[2] = 0xFF; // FIX: mark as unused for 2-byte message
      if (!arguments.silent && arguments.verbose)
    ...
    case SND_SEQ_EVENT_CHANPRESS:
      ...
      bytes[2] = 0xFF; // FIX: mark as unused for 2-byte message
      if (!arguments.silent && arguments.verbose)
        printf("Alsa    0x%02X Channel change     %03u %03u\n",
    ...
    case SND_SEQ_EVENT_PITCHBEND:
      ...
      if (!arguments.silent && arguments.verbose)
        printf("Alsa    0x%02X Pitch bend         %03u %5d\n", bytes[0] & 0xF0,
               bytes[0] & 0xF, ev->data.control.value);
      break;
    ...
    case SND_SEQ_EVENT_SYSEX: {
      ...
      for (i = 0; i < sysex_len; i++) {
        unsigned char *valPtr = (char *)ev->data.ext.ptr;
    ...
    default:
      // FIX: was `if (!arguments.verbose)` — inverted logic.
      // Warnings should print unless silenced, not unless verbose.
      if (!arguments.silent)
...
void write_midi_to_alsa(snd_seq_t *seq, int port_out_id, char *buf,
  ...
  /*
     ...
     -------------------------------------------------------------------
     source:
     http://ftp.ec.vanderbilt.edu/computermusic/musc216site/MIDI.Commands.html
     ...
     buf[1] --> param1
     buf[2] --> param2        (param2 not transmitted on program change or key
     press)
*/
  ...
  case 0xE0:
    ...
    snd_seq_ev_set_pitchbend(&ev, channel,
                             param1 - 8192); // in alsa MIDI we want signed int
    break;
...
#define BUF_SIZE                                                               \
  1024 // Size of the serial midi buffer - determines the maximum size of sysex
       // messages
...
int get_bytes_expected(int midicommand) {
  ...
  case 0xa0:
    return 2; // aftertouch
  case 0xb0:
    return 2; // continuous controller
  case 0xc0:
    return 1; // patch change
  case 0xd0:
    return 1; // channel pressure
  case 0xe0:
    return 2; // pitch bend
...
void *read_midi_from_serial_port(void *seq) {
  ...
  while (run) {
    ...
    while (bytesleft > 0) {
      ...
      // "transparent" and must not interrupt SysEx accumulation or channel
      // message parsing. Send them to ALSA immediately and loop back without
      // touching bytesleft, buflen, or buf.
      if (readbyte >= 0xF8) {
      ...
      if ((readbyte & 0x80) && (readbyte != 0xF7)) {
        ...
        bytesleft =
            get_bytes_expected(readbyte); // Determine the length of the message
      } else {
        if (buflen < BUF_SIZE - 1) // Guard against overflow
          buf[buflen++] = readbyte;
        if (readbyte == 0xF7)
          bytesleft = 0;
      }
    ...
    // FIX B: After a SysEx message, reset buf[0] and buflen so the next
    // iteration starts with a clean slate.
    //
    // Without this reset, buf[0] == 0xF0 persists into the next outer-loop
    // iteration. If the device then sends running-status data bytes (no
    // repeated status byte), they accumulate starting at buf[buflen] with
    // buf[0] still 0xF0. Eventually bytesleft hits zero and a garbled SysEx
    // blob is forwarded to ALSA — causing the observed "F0 63 00" corruption
    // instead of the expected "90 00 63" Note On.
    //
    // Per the MIDI spec, SysEx cancels running status, so discarding any
    // subsequent data-only bytes until the next status byte is correct.
    if (buf[0] == 0xF0) {
      buf[0] = 0x00; // clear stale SysEx status
      buflen = 0;    // start fresh for next message
      bytesleft = BUF_SIZE - 1;
    }
    // FIX F: Running status for channel messages (0x80–0xEF).
    // After delivering a complete channel message, keep the status byte in
    // buf[0] and reset buflen=1 so the next data-only pair (running status)
    // is accumulated and delivered correctly without waiting for BUF_SIZE-1
    // bytes to accumulate first.
    else if (buf[0] >= 0x80 && buf[0] < 0xF0) {
      buflen = 1;
      bytesleft = get_bytes_expected(buf[0]);
    }
    else {
      bytesleft = BUF_SIZE - 1;
    }
